...
def splitDatasets(datasets):
    ...
    for elem in datasets:
        if elem.purposeFlg == 'train':
            ...
            raise Exception(
                'Each dataset must be either a test, a train or a validation set.')
...
def createFolderStructure(seed):
    # make folders for seed and time, the rest will put into one folder for
    # each run and done by a file
    currentTime = datetime.datetime.now()

    cwd = os.getcwd()
    print(cwd)
    folderPathDate = os.path.join(
        os.getcwd(), 'results', str(
...
def pointsToTimeseries(points: np.array, windowStep=1):
    ...
    if len(points.shape) != 2:
        raise Exception(
            'Not clear how to produce the Timeseries, as the shape of the array is not like a matrix')
    else:
        ...
        largeMatrix = np.empty(shape=(numWindows,
                                      windowLength + (numWindows - 1) * windowStep))
        largeMatrix[:] = np.nan
        for i in range(numWindows):
            largeMatrix[i, i * windowStep:i *
                        windowStep + windowLength] = points[i]
...
def addFolder(folder, fold_object):
    ...
    newFolderPath = os.path.join(
        folder, fold_object.name[:5] + '_' + str(fold_object.obj_id)[:5])
    try:
...
def loadAE(folder):
    ...
    modelAE = autoencoder(
        ...
        # We are a little bit sloppy here, as we do not read torch.nn.ReLU()
        # from the parameters file. In this particular situation that is ok,
        # because SMT can only deal with ReLUs anyway
...
def loadAEParam(folder):
    ...
    aeParameters = loadParamsFromJson(
        os.path.join(folder, 'parameters_algorithm.txt'))
    return aeParameters
...
def loadAEState(folder, autoencoder):
    while 'autoencoder.pth' not in os.listdir(folder):
        # folder = folder[:folder.rfind('/')]
        folder = os.path.dirname(folder)
    autoencoder.module.load_state_dict(
        torch.load(os.path.join(folder, 'autoencoder.pth')))
...
def loadData(folder, datasetInstance):
    ...
    while fileName not in os.listdir(folder):
        # folder = folder[:folder.rfind('/')]
        folder = os.path.dirname(folder)
    datasetInstance.data = pd.read_csv(
        os.path.join(
            folder,
            fileName),
        header=0,
...
def solutionsToPoints(smtSolutions, smtVar):
    if smtSolutions is None or smtVar is None:
        return None
    else:
        smtSolutionModels = [smtSolution['model']
                             for smtSolution in smtSolutions]
        points = []
        for solution in smtSolutionModels:
            solPoint = []
            sortedSolution = sorted([(variable, solution[variable])
                                     for variable in solution], key=varKeyFunc)
            for var in smtVar:
                varPoint = []
