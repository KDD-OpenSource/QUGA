...
def executeExperiments(
	...
	algorithms, trainDatasets, testDatasets, validationDatasets, smts, resultsAE, resultsSMT, resultsJoined = decomposeObjects(objects)
	
	runFolder = createFolderStructure(settings.seed)


	# TMP
	trainDatasetAEFolders = ['/home/bboeing/bboeing_venv/AE_SMT/results/2020.03.30/24746/autoe_37382/twoSi_7e257']
	start = time.time()
	...
	if settings.experimentScope == 'ae_smt' or settings.experimentScope == 'smt':
		collectedResults = execSMTExperiments(trainDatasetAEFolders, smts, resultsSMT)
...
def execSMTExperiments(trainDatasetAEFolders, smts, resultsSMT):
	...
	for arguments in arg_list:
		collectedResults.append(pool.apply_async(execFixedFolder, args = arguments))
	pool.close()
	pool.join()
...
def execFixedFolder(folder, smt, resultSMT):
	...
	results.append(execFixedSMT(folder, smt, resultSMT, autoencoder, trainDataset))
	return results
...
def execFixedSMT(folder, smt, resultsSMT, autoencoder, trainDataset):
	...
	results = []
	for resultSMT in resultsSMT:
		results.append(execFixedResultSMT(smt, resultSMT, autoencoder, tmpFolderSmt, trainDataset))
	return results
