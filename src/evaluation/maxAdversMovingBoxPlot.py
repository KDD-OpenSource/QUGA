...
class maxAdversMovingBoxPlot(resultSMT):
    ...
    def calcResult(self, algorithm, trainDataset, smt):
        ...
        smtBBPosition = [(smt.abstractConstr['customBoundingBox'][i][1] +
                          smt.abstractConstr['customBoundingBox'][i][0]) /
                         2 for i in range(len(smt.abstractConstr['customBoundingBox']))]
        smtBBDistToZero = np.sqrt(
            np.square(
                np.array(
                    [
                        (smt.abstractConstr['customBoundingBox'][i][1] +
                         smt.abstractConstr['customBoundingBox'][i][0]) /
                        2 for i in range(
                            len(
                                smt.abstractConstr['customBoundingBox']))])).sum())
