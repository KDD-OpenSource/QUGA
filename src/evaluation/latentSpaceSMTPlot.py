...
class latentSpaceSMTPlot(resultSMT):
	...
	def calcResult(self, algorithm, trainDataset, smt):
		if 'adversAttack' in smt.abstractConstr:
			...
			trainLatentPoints = self.getTrainLatentPoints(trainDataset, algorithm)
	...
	def getResultFig(self, trainLatentPoints, maxAdversLatentPoint):
		...
		minDistToAdversLatent = euclidean_distances(trainLatentPoints, maxAdversLatentPoint).min()
		distInTrainLatentPoints = euclidean_distances(trainLatentPoints,trainLatentPoints)
