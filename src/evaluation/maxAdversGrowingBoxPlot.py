...
class maxAdversGrowingBoxPlot(resultSMT):
	...
	def __init__(self, name ='maxAdversGrowingBoxPlot', accuracy = 0.1):
		...
		self.collResults = pd.DataFrame(columns = ['algorithm', 'trainDataset', 'smtBBSize','maxAdversAttack'])
	...
	def calcResult(self, algorithm, trainDataset, smt):
		...
		smtBBSize = smt.abstractConstr['customBoundingBox'][0][1] - smt.abstractConstr['customBoundingBox'][0][0]
		print('smt_id: {}, smtBBSize: {}'.format(smt.obj_id, smtBBSize))
		print(type(smtBBSize))
		resultToAppend = pd.DataFrame(columns = ['algorithm', 'trainDataset', 'smtBBSize', 'maxAdversAttack'], data = [[algorithm, trainDataset, smtBBSize, maxAdversAttackTmp.AEError]])
	...
	def storeCollectedSMTResult(self, folder, figureDict):
		...
		plt.savefig(folder + '//'+'maxAdversAttackGrowing_'+str(figureDict['algorithm'].obj_id)[:4]+ '_' +str(figureDict['trainDataset'].obj_id)[:4]+'.png')
