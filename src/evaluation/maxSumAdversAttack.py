...
class maxSumAdversAttack(resultSMT):
	...
	def calcResult(self, algorithm, trainDataset, smt):
		if 'sumAdversAttack' in smt.abstractConstr:
			...
			maxSumAdversAttack = smt.getMaxSumAdversAttack(startValue = smt.abstractConstr['sumAdversAttack']['severity'], accuracy = self.accuracy, algorithm = algorithm, trainDataset = trainDataset)
