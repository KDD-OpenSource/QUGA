...
class theoMaxErrorEstAEParamPlot(resultSMT):
    ...
    def __init__(
            self,
            AEParam,
            name='theoMaxErrorEstAEParamPlot',
            accuracy=0.1):
    ...
    def calcResult(self, algorithm, trainDataset, smt):
        ...
        resultToAppend = pd.DataFrame(columns=['algorithm', 'trainDataset', 'smt_id', 'theoMaxErrorEst'], data=[
                                      [algorithm, trainDataset, smt.obj_id, maxAdversAttackTmp.theoMaxError]])
        ...
        for trainDataset in trainDatasets:
            for smt_id in self.collResults['smt_id'].unique():
                ...
                self.figures.append({'figure': self.calcCollectedSMTResult(
                    trainDataset, smt_id, maxTheoMaxErrorEst), 'smt_id': smt_id, 'trainDataset': trainDataset})
