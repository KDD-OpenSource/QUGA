...
class timeMaxErrorPlot(resultSMT):
	...
	def calcResult(self, algorithm, trainDataset, smt):
		...
		ax.plot(self.maxAdversAttack.calcTime, self.maxAdversAttack.result, color = 'red',marker = '+')	
		ax.plot(self.maxAdversAttack.calcTime, self.maxAdversAttack.AEError, color = 'blue',marker = '*')


		maxErrorEstTmpTimes = [elem['time'] for elem in self.maxErrorEst.result]
		maxErrorEstTmpResult = [elem['maxErrorEst'] for elem in self.maxErrorEst.result]
		maxLInftyErrorEstTmpResult = [elem['maxLInftyErrorEst'] for elem in self.maxErrorEst.result]
		...
		ax.plot(maxErrorEstTmpTimes, maxErrorEstTmpResult, color = 'blue', marker = '.')	
		ax.plot(maxErrorEstTmpTimes, maxLInftyErrorEstTmpResult, color = 'red', marker = '.')	
		self.result = fig
		# plt.show()
	...
	# we create a folder with the current timestamp. In this folder all the plots should be stored as files
		...
		with open(file, 'w') as file:
			file.write('The severity of this solution is: {}'.format(self.maxAdversAttack.result))
			file.write('\n')
			file.write('The MSE on this point is: {}'.format(self.maxAdversAttack.AEError))
			file.write('\n')
			file.write('The theoretical maximum Error for the severity is: {}'.format(self.maxAdversAttack.theoMaxError))
			file.write('\n')
			file.write('The calculation Time is: {}'.format(self.maxAdversAttack.calcTime))
		os.chdir(cwd)
