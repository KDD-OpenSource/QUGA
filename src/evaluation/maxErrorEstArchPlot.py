...
class maxErrorEstAEParamPlot(resultAE):
    ...
    def calcResult(self, algorithm, trainDataset, testDataset):
        ...
        maxErrorEstTmp = pd.DataFrame(
            [np.square(np.subtract(uniformSample, prediction)).mean(axis=1).max()])
    ...
    def storeCollectedAEResult(self, folder, figureDict):
        ...
        plt.savefig(folder +
                    ...
                    '_' +
                    str(figureDict['boundingBox'])[:5] +
                    '_' +
                    str(figureDict['sampleSize']) +
                    '.png')
        # plt.close('all')
