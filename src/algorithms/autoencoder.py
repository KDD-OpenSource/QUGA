...
class autoencoder(algorithm):
    ...
    def predict(self, data: pd.DataFrame):
        data = data.values.astype(np.float32)
        # Note, that we have to add [:,0,:], because we have a list (first :)
        # of tensors. These tensors are of the form [[val_1,val_2,...]],
        # because there is a wrapper in order to account for batch-calculation.
        # With the '0' we get rid of this wrapper (as we are predicting one by
        # one and not in batches)
        dataPredicted = np.array([self.module(record)[1].detach(
        ).numpy() for record in DataLoader(data)])[:, 0, :]
    ...
    def getLatentSpace(self, data: pd.DataFrame):
        ...
        latentSpaceDF = pd.DataFrame(
            [point.detach().numpy().flatten() for point in latentSpace])
    ...
    def getAEResults(self, data: pd.DataFrame):
        ...
        resultDF = pd.DataFrame(
            [point.detach().numpy().flatten() for point in result])
        # return sequencesOutput
...
class autoencoderModule(nn.Module):
    def __init__(self, architecture, bias, activationFct, initialization):
        ...
        seqEncLayers = [list(elem) for elem in list(
            zip(encLayers[:-1], encLayers[1:]))]
        seqDecLayers = [list(elem) for elem in list(
            zip(decLayers[:-1], decLayers[1:]))]
        encLayers = np.array([[nn.Linear(int(a),
                                         int(b),
                                         bias=self.bias),
                               self.activationFct] for a,
                              b in seqEncLayers]).flatten()
        decLayers = np.array([[nn.Linear(int(a),
                                         int(b),
                                         bias=self.bias),
                               self.activationFct] for a,
                              b in seqDecLayers]).flatten()[:-1]
