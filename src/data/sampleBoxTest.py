...
class sampleBoxTest(dataset):
	...
	def __init__(self, name, seed, purposeFlg, box, numPoints, windowStep = 1):
		super(sampleBoxTest, self).__init__(name=name, seed=seed, purposeFlg = purposeFlg, windowStep = windowStep,tsFlg = False)
		uniformSample = np.random.uniform(low = np.array(box).transpose()[0], high = np.array(box).transpose()[1], size = (numPoints,len(box)))
