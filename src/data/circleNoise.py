...
class circleNoise(dataset):
    ...
    def __init__(
        ...
        gaussiansDimsCircle = np.random.normal(
            0, 1, size=(numDatapoints, self.numDimsCircle))
        ringDimsCircle = gaussiansDimsCircle / 10 + gaussiansDimsCircle * \
            1 / (np.linalg.norm(gaussiansDimsCircle, axis=1).reshape(-1, 1))
        if self.numDimsTotal > self.numDimsCircle:
            gaussiansRest = np.random.normal(
                0,
                1,
                size=(
                    numDatapoints,
