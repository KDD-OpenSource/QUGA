...
class dataset():
    ...
    def timeseriesToPoints(self, windowLength):
        if self.tsFlg is False:
            ...
            if (flattenedData.shape[0] - windowLength) % self.windowStep == 0:
                ...
                raise Exception(
                    'Could not divide time series into autoencoder data, because windowLength, windowLength and the length of the Timeseries do not fit together.')
    ...
    def pointsToTimeseries(self):
        ...
        if self.tsFlg is True:
            ...
            if self.windowStep > windowLength:
                raise Exception(
                    'Your time window is shorter than the window step. You will end up with a TS with nan-values.')
            numWindows = self.data.shape[0]
            # largeMatrix = np.empty(shape =(numWindows, numWindows+windowLength-1))
            largeMatrix = np.empty(
                shape=(numWindows, windowLength + (numWindows - 1) * self.windowStep))
            largeMatrix[:] = np.nan
            for i in range(numWindows):
                largeMatrix[i, i * self.windowStep:i *
                            self.windowStep + windowLength] = self.data.iloc[i]
    ...
    def saveData(self, folder):
        ...
        if self.purposeFlg == 'train':
            # with open(folder + '/' + 'parameters_trainDataset.txt', 'w') as
            # jsonFile:
            ...
            # with open(folder + '/' + 'parameters_testDataset.txt', 'w') as
            # jsonFile:
            with open(os.path.join(folder, 'parameters_testDataset.txt'), 'w') as jsonFile:
                json.dump(dataDict, jsonFile, indent=0)
        else:
            # raise Exception(f'The purposeFlg of the dataset {self.data.name}
            # is neither train nor test')
            raise Exception(
                'The purposeFlg of the dataset is neither train nor test')
