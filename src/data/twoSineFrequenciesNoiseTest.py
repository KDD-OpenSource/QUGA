...
class twoSineFrequenciesNoiseTest(dataset):
    ...
    # def __init__(self, name, seed, purposeFlg, numCycles0, cycleLength0,
    # numCycles1, cycleLength1, var, windowStep = 1):
            ...
            for splitInd in range(len(cycleSplits)):
                ...
                yAxis = np.sin(
                    np.arange(
                        0,
                        2 * np.pi,
                        (2 * np.pi) / cycleLengths[cycleInd]))
                yAxis.reshape(cycleSplits[cycleInd], -1)
                if len(yAxis.shape) == 1:
                    yAxis = np.array([yAxis])

                for clusterSizeInd in range(len(numPerCluster[splitInd])):
                    cyclesPoints = pd.DataFrame(
                        data=[
                            yAxis[clusterSizeInd] +
                            np.random.normal(
                                0,
                                var,
                                int(
                                    cycleLengths[cycleInd] /
                                    cycleLengths[cycleInd])) for i in range(
                                numPerCluster[splitInd][clusterSizeInd])])
                    totalPoints = pd.concat([totalPoints, cyclesPoints]).sample(
