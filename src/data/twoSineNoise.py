...
class twoSineNoise(dataset):
	...
	def __init__(self, name, seed, purposeFlg, wavelength, num0, numPi, var):
		...
		yAxisShift = np.sin(np.arange(np.pi, np.pi + 2*np.pi, (2*np.pi)/wavelength))
		num0Points = pd.DataFrame(data=[yAxis + np.random.normal(0,var,wavelength) for i in range(num0)])
		numPiPoints = pd.DataFrame(data=[yAxisShift + np.random.normal(0,var,wavelength) for i in range(numPi)])
		self.data = pd.concat([num0Points, numPiPoints]).sample(frac = 1).reset_index(drop = True)